// src/main/java/ru/soat/ohapp/saas/notify/SmsNotificationSender.java
...
public class SmsNotificationSender implements NotificationSender {
    ...
    /** Включение реальной отправки SMS (по умолчанию выключено для dev/test). */
    @Value("${ohapp.notify.sms.enabled:false}")
    ...
    public void send(Reminder r) {
        ...
        if (to == null || to.isBlank() || !to.matches(E164)) {
            throw new IllegalArgumentException("Invalid SMS recipient (must be E.164): " + to);
        }

        if (!enabled) {
            log.info("[DEV] SMS disabled. Would send to='{}' text='{}'", maskPhone(to), text);
            return;
        }

        // TODO: интеграция с провайдером (Twilio/Infobip/SMSC.ru и т.п.)
        // Пример псевдокода:
        // smsClient.send(new SmsMessage(to, text));
        log.info("[SMS] Sent to='{}' text='{}'", maskPhone(to), text);
    }
